    ...
    std::vector<T>& allDs)
{
  ...
  auto [tActive, gaitRamp, gaitRampRate] = actuationProfile(t, p.restTime, p.rampTime);
  T ampScale = ampRamp * gaitRamp;
  T ampRate  = ampRamp * gaitRampRate;
  ...
  std::vector<T> vyDefL(nSp), vxDefL(nSp, 0.0);
  ...
  // Tangent and normal
  T dsSpine = scale * L / (nSp - 1);
  for (int i = 0; i < nSp; ++i) {
    T dxDs = scale;
    // d(A*sin(phase))/ds * scale
    T dAds = 0.0;
    if (i > 0 && i < nSp - 1) {
      dAds = (Aval[i+1] - Aval[i-1]) / (2.0 * L / (nSp - 1));
    } else if (i == 0) {
      dAds = (Aval[1] - Aval[0]) / (L / (nSp - 1));
    } else {
      dAds = (Aval[nSp-1] - Aval[nSp-2]) / (L / (nSp - 1));
    }
    T dyDs = scale * (Aval[i] * std::cos(phase[i]) * (pi2 / lam) + dAds * std::sin(phase[i]));

    T tlen = std::sqrt(dxDs * dxDs + dyDs * dyDs);
    txV[i] = dxDs / tlen;
    tyV[i] = dyDs / tlen;
    nxN[i] = -tyV[i];
    nyN[i] =  txV[i];

    // Deformation velocity in local frame  [lu / step].
    //
    // The body-wave displacement is  y(s,t) = A(s,t) · sin(phase(s,t)).
    // Its time-derivative ∂y/∂t lives in [lu / s] because freq [Hz] and
    // ampRate [1/s] are defined in the external physical-time coordinate.
    // Multiplying by dtLbm [s / step] converts to the per-step rate that
    // the IBM marker desired-velocity expects (same units as Vx, Vy).
    vyDefL[i] = scale * Abase[i] * (
      ampRate * std::sin(phase[i]) - ampScale * pi2 * freq * std::cos(phase[i])
    ) * dtLbm;
  }
  ...
  // Semicircular caps
  int nHeadCap = std::max(static_cast<int>(std::ceil(M_PI * r / dsSpine)), 8);
  int nTailCap = std::max(static_cast<int>(std::ceil(M_PI * r / dsSpine)), 8);
  T dsHeadCap = M_PI * r / nHeadCap;
  T dsTailCap = M_PI * r / nTailCap;

  // Head cap
  std::vector<T> headCapX(nHeadCap + 1), headCapY(nHeadCap + 1);
  for (int i = 0; i <= nHeadCap; ++i) {
    T phi = M_PI * i / nHeadCap;
  ...
  int nTotal = nSp + (nTailCap - 1) + nSp + (nHeadCap - 1);
  std::vector<T> locX(nTotal), locY(nTotal);
  std::vector<T> vxDef(nTotal), vyDef(nTotal);
  allDs.resize(nTotal);
  ...
  for (int i = 0; i < nSp; ++i) {
    ...
    vxDef[idx] = vxDefL[i]; vyDef[idx] = vyDefL[i];
    allDs[idx] = dsSpine;
    idx++;
  }
  ...
  for (int i = 1; i < nTailCap; ++i) {
    ...
    vxDef[idx] = vxDefL[nSp-1]; vyDef[idx] = vyDefL[nSp-1];
    allDs[idx] = dsTailCap;
    idx++;
  }
  // Lower side (reversed)
  ...
  for (int i = 1; i < nHeadCap; ++i) {
    ...
    vxDef[idx] = vxDefL[0]; vyDef[idx] = vyDefL[0];
    allDs[idx] = dsHeadCap;
    idx++;
  }
  ...
  globX.resize(nTotal);
  globY.resize(nTotal);
  vxDefGlob.resize(nTotal);
  vyDefGlob.resize(nTotal);

  for (int i = 0; i < nTotal; ++i) {
    globX[i] = cosT * locX[i] - sinT * locY[i] + xCm;
    globY[i] = sinT * locX[i] + cosT * locY[i] + yCm;
    vxDefGlob[i] = cosT * vxDef[i] - sinT * vyDef[i];
    vyDefGlob[i] = sinT * vxDef[i] + cosT * vyDef[i];
  }
}
